...
load("/PATH/TO/SEQUENCE/DATA/mydada_16S.V4.Rdata")
...
fnRs <- sort(list.files(path, pattern="_R2_clean.fastq", full.names = TRUE)); save.image(file = "mydada_16S.V4.Rdata")

fnFs

# Extract sample names, assuming filenames have format: SAMPLENAME_XXX.fastq
sample.names <- sapply(strsplit(basename(fnFs), "_R1"), `[`, 1); save.image(file = "mydada_16S.V4.Rdata") #pattern where you want to split the name
sample.names
...
ggsave(plot1,filename = "16S.V4_pretrim_DADA2_F_quality.pdf", width=15, height=12, dpi=600)
ggsave(plot2,filename = "16S.V4_pretrim_DADA2_R_quality.pdf", width=15, height=12, dpi=600)
...
# truncLen=c(240,160) -- trim F reads to 240 bp, trim R reads to 160 bp
...
ggsave(plot_error,filename = "16S.V4_errormodel_DADA2.pdf", width=15, height=15, dpi=600)
...
dim(seqtab)
seqtab3 <- seqtab[,nchar(colnames(seqtab)) %in% 250:285]
...
write.table(seqtab,"16S.V4_Read_Length_Counts_dada2.txt",sep="\t",row.names=TRUE,col.names=TRUE); save.image(file = "mydada_16S.V4.Rdata")
...
compare_reads_plot = ggdensity(rowSums(seqtab3), fill = "blue4", alpha = 0.7); save.image(file = "mydada_16S.V4.Rdata")
pdf(file = "16S.V4_compare_plots.pdf", height = 10, width = 12) # Make empty pdf
...
#### Remove Chimeras ####
seqtab.nochim <- removeBimeraDenovo(seqtab3, method="consensus", multithread=TRUE, verbose=TRUE); save.image(file = "mydada_16S.V4.Rdata")
...
head(track)
write.table(track,"16S.V4_tracking_reads_dada2.txt",sep="\t",row.names=TRUE,col.names=TRUE); save.image(file = "mydada_16S.V4.Rdata")
...
# dada2 package also implements a method to make species level assignments based on exact matching between ASVs and sequenced reference strains. Recent analysis suggests that exact matching (or 100% identity) is the only appropriate way to assign species to 16S gene fragments.
...
#taxa <- addSpecies(taxa, "/bigdata/aronsonlab/shared/DADA2_Silva_Files/silva_species_assignment_v138.fa.gz"); save.image(file = "mydada_16S.V4.Rdata")
...
write(asv_fasta, "16S.V4_ASVs_dada2.fa") # write fasta file
write.table(asv_fasta,"16S.V4_ASVs_dada2.txt",sep="\t",row.names=TRUE,col.names=TRUE)

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "16S.V4_ASVs_Counts_dada2.tsv", sep="\t", quote=F, col.names=NA)
write.table(asv_tab,"16S.V4_ASVs_Counts_dada2.txt",sep="\t",row.names=TRUE,col.names=TRUE)
