...
load("/PATH/TO/SEQUENCE/DATA/mydada_16S.V3.Rdata")
...
fnRs <- sort(list.files(path, pattern="_R2_clean.fastq", full.names = TRUE)); save.image(file = "mydada_16S.V3.Rdata")

fnFs

# Extract sample names, assuming filenames have format: SAMPLENAME_XXX.fastq
sample.names <- sapply(strsplit(basename(fnFs), "_R1"), `[`, 1); save.image(file = "mydada_16S.V3.Rdata") #pattern where you want to split the name
sample.names
...
names(filtRs) <- sample.names; save.image(file = "mydada_16S.V3.Rdata")
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(130,120),
                     maxN=0, maxEE=c(2,3), truncQ=2, rm.phix=TRUE,
                     compress=TRUE, multithread=TRUE); save.image(file = "mydada_16S.V3.Rdata") # On Windows set multithread=FALSE
...
errF <- learnErrors(filtFs, multithread=TRUE); save.image(file = "mydada_16S.V3.Rdata")
errR <- learnErrors(filtRs, multithread=TRUE); save.image(file = "mydada_16S.V3.Rdata")
...
dadaFs <- dada(filtFs, err=errF, multithread=TRUE, pool=TRUE); save.image(file = "mydada_16S.V3.Rdata") # pseudo pooling is computationally more efficient but similar in results to pooling; pool = True will pool samples together before sample inference
dadaRs <- dada(filtRs, err=errR, multithread=TRUE, pool=TRUE); save.image(file = "mydada_16S.V3.Rdata")
...
mergers <- mergePairs(dadaFs, filtFs, dadaRs, filtRs, verbose=TRUE); save.image(file = "mydada_16S.V3.Rdata")
# pick up where you left off vvv
load("/bigdata/aronsonlab/shared/Zymo2021/BREATHE_Amplicon/Zymo_Run_8.06.21/Mitochondrial_Blockers/Trimmed_Seqs/16S.V3_Trimmed/mydada_16S.V3.Rdata")
...
write.table(seqtab,"16S.V3_Read_Length_Counts_dada2.txt",sep="\t",row.names=TRUE,col.names=TRUE); save.image(file = "mydada_16S.V3.Rdata")
...
track <- cbind(out, sapply(dadaFs, getN), sapply(dadaRs, getN), sapply(mergers, getN), rowSums(seqtab.nochim)); save.image(file = "mydada_16S.V3.Rdata")
...
head(track)
write.table(track,"16S.V3_tracking_reads_dada2.txt",sep="\t",row.names=TRUE,col.names=TRUE); save.image(file = "mydada_16S.V3.Rdata")
...
# dada2 package also implements a method to make species level assignments based on exact matching between ASVs and sequenced reference strains. Recent analysis suggests that exact matching (or 100% identity) is the only appropriate way to assign species to 16S.V3 gene fragments.
# Currently, species-assignment training fastas are available for the Silva and RDP 16S.V3 databases. To follow the optional species addition step, download the silva_species_assignment_v132.fa.gz file, and place it in the directory with the fastq files.
#taxa <- addSpecies(taxa, "/bigdata/aronsonlab/shared/DADA2_Silva_Files/silva_species_assignment_v138.fa.gz"); save.image(file = "mydada_16S.V3.Rdata")
...
save.image(file = "mydada_16S.V3.Rdata")
...
asv_tab_veg<-t(asv_tab)
write.table(asv_tab_veg, "16S.V3_ASVs_Table_dada2.tsv", sep="\t", quote=F, col.names=NA)
